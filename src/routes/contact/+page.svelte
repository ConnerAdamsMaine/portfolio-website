...
<section class="section-pad">
	<div class="grid gap-10 lg:grid-cols-[1.1fr_0.9fr]">
		<div class="space-y-5">
			...
			<h1 class="text-4xl font-semibold text-white sm:text-5xl">{data.siteSettings.contactTitle}</h1>
			<p class="text-lg text-ink-200">{data.siteSettings.contactBody}</p>
			<div class="grid gap-4 sm:grid-cols-2">
				<div class="glass p-5">
					<p class="text-xs uppercase tracking-[0.2em] text-ink-200">Email</p>
					<p class="mt-2 text-lg font-semibold text-white">{data.siteSettings.contactEmail}</p>
				</div>
				<div class="glass p-5">
					<p class="text-xs uppercase tracking-[0.2em] text-ink-200">Location</p>
					<p class="mt-2 text-lg font-semibold text-white">Remote / Worldwide</p>
				</div>
			</div>
		</div>
		<MotionReveal className="glass p-8">
			<form class="space-y-4" method="POST" action="?/send" on:submit={handleSubmit}>
				<div>
					<label class="text-xs font-semibold uppercase tracking-[0.2em] text-ink-200" for="name">Name</label>
					<input
						id="name"
						name="name"
				...
				<div>
					<label class="text-xs font-semibold uppercase tracking-[0.2em] text-ink-200" for="email">Email</label>
					<input
						id="email"
						name="email"
						class="mt-2 w-full rounded-2xl border border-ink-200/40 bg-white/5 px-4 py-3 text-sm text-white"
				...
				<div>
					<label class="text-xs font-semibold uppercase tracking-[0.2em] text-ink-200" for="scope">Scope</label>
					<textarea
						id="scope"
						name="scope"
