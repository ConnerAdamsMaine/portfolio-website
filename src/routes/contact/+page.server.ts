...
export const actions: Actions = {
	send: async (event) => {
		...
		if (!(await rateLimit(`contact-form:${ip}`, { windowMs: 10 * 60 * 1000, max: 6 }))) {
		...
		if (scope.length > MAX_SCOPE_LENGTH) {
			return fail(400, { message: 'Project scope is too long.' });
		}
		...
		try {
			...
			return fail(500, { message: 'Unable to save your message right now. Please try again shortly.' });
		}
